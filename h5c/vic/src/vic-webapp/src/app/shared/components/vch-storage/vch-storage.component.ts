...
export class VchStorageComponent extends VchComponentBase implements OnInit {
  ...
  protected setFormValues(model: VchUiStorage) {
    console.log('setFormValues:', model);
  ...
  protected updateCurrentModel() {
    if (this.form.valid) {
      ...
      this.model.volumeStore = this.form.get('volumeStore').value
        .filter((vol: VchUiStorageVolumeStore) => vol.volDatastore);
      ...
      console.log('updateCurrentModel:', this.model);
  ...
  onCommit(): Observable<{storageCapacity: VchUiStorage}> {
    ...
    console.log('storage:', this.model);
  ...
  onPageLoad() {
    ...
    console.log('onPageLoad1:')
    this.form.get('enableAnonymousVolumes').valueChanges
      .subscribe(v => {
        const volStores = this.form.get('volumeStore') as FormArray;
        console.log('volStores:', volStores.length);
        if (v) {
